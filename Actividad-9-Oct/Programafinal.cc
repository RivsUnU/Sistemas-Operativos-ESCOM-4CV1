//Programa final - Modificado por Caldo de Gallina 4CV1
#include <stdio.h>
#include <pthread.h>
#include <semaphore.h>
#include <stdlib.h>
...
#define SWAP_TIME 1  // Tiempo que un hilo permanece en estado swap
...
typedef struct {
    ...
    int *active;     // Estado del hilo (activo/inactivo)
} ThreadData;
...
void planificador(int hilosActivos[], int n) {
    ...
    while (n > 0) {
        if (hilosActivos[i] > 0) {
            ...
            // Si el hilo ha usado su tiempo y aún no ha terminado, se mueve a swap
            if (hilosActivos[i] == 0) {
                n--;
                // Simular el swap
                printf("Hilo %d en swap.\n", i);
                sleep(SWAP_TIME);  // Simula el tiempo de swap
                printf("Hilo %d vuelve a la ejecución.\n", i);
                hilosActivos[i] = 2;  // Reanudar el hilo, por ejemplo, con un nuevo quantum
            }
        }
...
int main() {
    ...
    int hilosActivos[MAX_HILOS] = {2, 2, 2, 2}; // Simula la carga para Round Robin
    ...
    for (i = 0; i < MAX_HILOS; i++) {
        ...
        datos[i].active = &hilosActivos[i]; // Pasar el estado activo del hilo
